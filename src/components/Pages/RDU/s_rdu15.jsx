// 9.1 ร้อยละของผู้ป่วยนอกโรคหืดที่ได้รับยา inhaled corticosteroid (PDx)
...
export default function S_rdu15() {
  ...
  const excelData = filteredData.map(item => ({
     "หน่วยบริการ": `${item.hospcode}:${item.hospname}`,
        ...
        "ไตรมาส 1": {
            "B": Helper.formatNumber(item.targetq1),
            "A": Helper.formatNumber(item.resultq1),
            "ร้อยละ": item.targetq1 > 0 ? ((item.resultq1 / item.targetq1) * 100).toFixed(2) : "0"
        },
        "ไตรมาส 2": {
            "B": Helper.formatNumber(item.targetq2),
            "A": Helper.formatNumber(item.resultq2),
            "ร้อยละ": item.targetq2 > 0 ? ((item.resultq2 / item.targetq2) * 100).toFixed(2) : "0"
        },
        "ไตรมาส 3": {
            "B": Helper.formatNumber(item.targetq3),
            "A": Helper.formatNumber(item.resultq3),
            "ร้อยละ": item.targetq3 > 0 ? ((item.resultq3 / item.targetq3) * 100).toFixed(2) : "0"
        },
        "ไตรมาส 4": {
            "B": Helper.formatNumber(item.targetq4),
            "A": Helper.formatNumber(item.resultq4),
            "ร้อยละ": item.targetq4 > 0 ? ((item.resultq4 / item.targetq4) * 100).toFixed(2) : "0"
        }
    }));
        ...
        <div className="container mx-auto px-5 py-10">
          ...
          <div className="my-8 bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800 mb-6">
              ร้อยละของผู้ป่วยนอกโรคหืดที่ได้รับยา inhaled corticosteroid (PDx) ปีงบประมาณ {selectedYear}
            </h2>
            {loading ? (
          ...
          <div className="w-full overflow-x-auto shadow-md rounded-lg">
            <h2 className="text-lg font-semibold text-gray-600 mb-4">
              ร้อยละของผู้ป่วยนอกโรคหืดที่ได้รับยา inhaled corticosteroid (PDx)
            </h2>
                  ...
                  filteredData.map((data, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.target)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.result)}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-green-600">
                        {data.target > 0
                          ? (
                              (data.result / data.target) * 100).toFixed(2) : "0.00"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.targetq1)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.resultq1)}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-green-600">
                        {data.targetq1 > 0
                          ? (
                              (data.resultq1 / data.targetq1) * 100).toFixed(2) : "0.00"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.targetq2)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.resultq2)}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-green-600">
                        {data.targetq2 > 0
                          ? (
                              (data.resultq2 / data.targetq2) * 100).toFixed(2) : "0.00"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.targetq3)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.resultq3)}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-green-600">
                        {data.targetq3 > 0
                          ? (
                              (data.resultq3 / data.targetq3) * 100).toFixed(2) : "0.00"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.targetq4)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {Helper.formatNumber(data.resultq4)}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-green-600">
                        {data.targetq4 > 0
                          ? (
                              (data.resultq4 / data.targetq4) * 100).toFixed(2) : "0.00"}
                      </td>
                    </tr>
                  ))
                ) : (
            ...
            <div className="p-2 text-xs text-left font-semibold text-green-600">
              B หมายถึง จำนวนผู้ป่วยนอกโรคหืดทั้งหมด(PDX)
            </div>
            <div className="p-2 text-xs text-left font-semibold text-green-600">
              A หมายถึง จำนวนผู้ป่วยนอกโรคหืดที่ได้รับยา inhaled corticosteroid
            </div>
          </div>
        </div>
        <Footer />
