...
const headers = [
  "หน่วยบริการ",
  {
    title: "3.1 บาดแผลสดจากอุบัติเหตุ ทั้งหมด",
    children: ["B", "A", "ร้อยละ"]
  },
  {
    title: "3.2 บาดแผลสดจากอุบัติเหตุ (ไม่นับรวมแผลสัตว์กัด แผลลึกถึงกล้ามเนื้อ และแผลไฟไหม้น้ำร้อนลวก)",
    children: ["B", "A", "ร้อยละ"]
  }
];

export default function S_rdu8() {
  ...
  const excelData = filteredData.map(item => ({
     "หน่วยบริการ": `${item.hospcode}:${item.hospname}`,
        "3.1 บาดแผลสดจากอุบัติเหตุ ทั้งหมด": {
        ...
        "3.2 บาดแผลสดจากอุบัติเหตุ (ไม่นับรวมแผลสัตว์กัด แผลลึกถึงกล้ามเนื้อ และแผลไฟไหม้น้ำร้อนลวก)": {
            "B": Helper.formatNumber(item.target1),
            "A": Helper.formatNumber(item.result1),
            "ร้อยละ": item.target1 > 0 ? ((item.result1 / item.target1) * 100).toFixed(2) : "0"
        }
    }));
        ...
        const seriesData1 = [
          {
            name: "ร้อยละ",
            data: dataWithNames.map(item => 
              item.target1 > 0 ? parseFloat((item.result1 / item.target1) * 100) : 0
            )
          }
        ];
          ...
          <div className="container mx-auto px-5 py-10">
            ...
            <div className="my-8 bg-white p-6 rounded-lg shadow-md border border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800 mb-6">
                    กราฟแสดงร้อยละการใช้ยาปฏิชีวนะในบาดแผลสดจากอุบัติเหตุ ระดับโรงพยาบาล (FTW) ประจำปีงบประมาณ {selectedYear}
                </h2>
            ...
            <div className="w-full overflow-x-auto shadow-md rounded-lg">
              <h2 className="text-lg font-semibold text-gray-600 mb-4">ร้อยละการใช้ยาปฏิชีวนะในบาดแผลสดจากอุบัติเหตุ ระดับโรงพยาบาล (FTW)</h2>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((data, index) => (
                      <tr key={index} className="bg-white border-b hover:bg-gray-200">
                        ...
                        <td className="px-6 py-4 text-right">{Helper.formatNumber(data.target1)}</td>
                        <td className="px-6 py-4 text-right">{Helper.formatNumber(data.result1)}</td>                        
                        <td className="px-6 py-4 text-right font-bold text-green-600">
                          {data.target1 > 0 ? ((data.result1 / data.target1) * 100).toFixed(2) : "0.00"}
                        </td>
                      </tr>
                    ))
                  ) : (
              ...
              <div className='p-2 text-xs text-left font-semibold text-green-600'>B หมายถึง จำนวนครั้งการมารับบริการผู้ป่วยนอกของผู้ป่วยบาดแผลสดจากอุบัติเหตุ</div>
              <div className='p-2 text-xs text-left font-semibold text-green-600'>A หมายถึง จำนวนครั้งการมารับบริการผู้ป่วยนอกของผู้ป่วยบาดแผลสดจากอุบัติเหตุ ที่ได้รับยาปฏิชีวนะ</div>
            </div>
          </div>
          <Footer />  
    </> 
